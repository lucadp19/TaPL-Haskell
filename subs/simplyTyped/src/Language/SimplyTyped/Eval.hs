...
shift d = go 0
  where 
    ...
    go c = \case
        ...
        Lam name typ t -> Lam name typ $ go (c+1) t
...
step (App (Lam name ty t1) t2) 
    | isValue t2 = pure $ substTop t2 t1
    | otherwise  = App (Lam name ty t1) <$> step t2
